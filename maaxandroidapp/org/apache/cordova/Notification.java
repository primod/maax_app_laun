  ...
  extends CordovaPlugin
{
  ...
  public void confirm(final String paramString1, final String paramString2, String paramString3, final CallbackContext paramCallbackContext)
  {
    try
    {
      Runnable local2 = new Runnable()
      {
        public void run()
        {
          ...
          if (this.val$fButtons.length > 0) {
            localBuilder.setNegativeButton(this.val$fButtons[0], new DialogInterface.OnClickListener()
            {
              public void onClick(DialogInterface paramAnonymous2DialogInterface, int paramAnonymous2Int)
              {
                paramAnonymous2DialogInterface.dismiss();
          ...
          if (this.val$fButtons.length > 1) {
            localBuilder.setNeutralButton(this.val$fButtons[1], new DialogInterface.OnClickListener()
            {
              public void onClick(DialogInterface paramAnonymous2DialogInterface, int paramAnonymous2Int)
              {
                paramAnonymous2DialogInterface.dismiss();
          ...
          if (this.val$fButtons.length > 2) {
            localBuilder.setPositiveButton(this.val$fButtons[2], new DialogInterface.OnClickListener()
            {
              public void onClick(DialogInterface paramAnonymous2DialogInterface, int paramAnonymous2Int)
              {
                paramAnonymous2DialogInterface.dismiss();
    ...
    throws JSONException
  {
    ...
    for (;;)
    {
      ...
      else
      {
        ...
        {
          confirm(paramJSONArray.getString(0), paramJSONArray.getString(1), paramJSONArray.getString(2), paramCallbackContext);
          return true;
        }
      ...
      Runnable local4 = new Runnable()
      {
        public void run()
        {
          ...
          jdField_this.progressDialog.setOnCancelListener(new DialogInterface.OnCancelListener()
          {
            public void onCancel(DialogInterface paramAnonymous2DialogInterface)
            {
              Notification.4.this.val$notification.progressDialog = null;
            }
          });
