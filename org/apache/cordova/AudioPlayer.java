  ...
  implements MediaPlayer.OnCompletionListener, MediaPlayer.OnPreparedListener, MediaPlayer.OnErrorListener
{
  ...
  public void moveFile(String paramString)
  {
    ...
    if (Environment.getExternalStorageState().equals("mounted"))
    {
      localFile.renameTo(new File(Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + paramString));
      return;
    }
    localFile.renameTo(new File("/data/data/" + this.handler.cordova.getActivity().getPackageName() + "/cache/" + paramString));
  }
